describe('Dokumenty sprzedaży', function () {
    it('Faktura', function () {
        ...
        cy.get(DataInput + '[ContractorDetail][name]"]').type('kontrahent' + RandomNumbContractor).get('label').contains('Nabywca').click()
            .get(DataInput + '[ContractorDetail][zip]"]').type(RandomNumbContractor) //.get(DataInput + '[ContractorDetail][zip]"]').contains(RandomNumbContractor)
            .get(DataInput + '[ContractorDetail][city]"]').clear().type('Miasto ' + RandomNumbContractor).get(DataInput + '[ContractorDetail][city]"]').should('have.value', 'Miasto ' + RandomNumbContractor)
            .get(DataInput + '[ContractorDetail][street]"]').type('Ulica ' + RandomNumbContractor).get(DataInput + '[ContractorDetail][street]"]').should('have.value', 'Ulica ' + RandomNumbContractor)
            .get('label div.input-append.input-prepend.inputer.input-select:first').click()
            .get('span').contains('Inny').click()
            .get(DataInput + '[ContractorDetail][nip]"]').type(RandomNumbContractor).get(DataInput + '[ContractorDetail][nip]"]').should('have.value','' + RandomNumbContractor)
        ...
        for (var PositionIndex=0; PositionIndex < InvoiceContentCount; ) {
            ...
            cy.get(DataInput + '[InvoiceContent][' + PositionIndex + '][name]"]').type(RandomNumb)
                .get(DataInput + '[InvoiceContent][' + PositionIndex + '][count]"]').type(RandomNumb)
                .get(DataInput + '[InvoiceContent][' + PositionIndex + '][price]"]').type(RandomNumb)
                .get('label').contains('Nabywca').click()
            //dodawanie nowej pozycji
            cy.get('a.btn.btn-mini.add-row').click()
            PositionIndex++;
        }
